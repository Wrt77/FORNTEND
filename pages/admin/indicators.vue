...
const headers = [
    ...
    { title: 'ชื่อตัวชี้วัด', key: 'name_th', soratable: true },
    { title: 'หัวข้อ', key: 'topic_name', soratable: true },
    { title: 'ประเภท', key: 'type', sortable: true },
    { title: 'นั้าหนัก', key: 'weight', sortable: true, align: 'center'},
    { title: 'สถานะ', key: 'active', soratable: true, align: 'center' },
    { title: 'จัดการ', key: 'actions', soratable: false, align: 'center' }
] 
...
const formTitle = computed(() => editedIndex.value === -1 ? 'เพิ่มต่าตัวชี้วัด' : 'แก้ไขตัวชี้วัด' )
...
async function fetchItems() {
    ...
    try {
        ...
        console.log('🔍 RAW RESPNSE:', res)

        let dataArry = Array.isArray(res) ? res : (res?.items || [])
...
<template>
    <v-container fluid>
        <h1 class="text-h4 md-4">Indicators</h1>
        <p class="text-subtitle-1 mb-6">จัดการตัวชี้วัดการประเมิน</p>

        <v-alert v-if="successMsg" type="success" dismissable class="mb-4" @click:close="errorMsg = ''">
            {{ successMsg }}
        </v-alert>
